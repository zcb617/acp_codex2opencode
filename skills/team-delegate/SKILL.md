...
9. **计划必须对齐方案来源。** Planning 不能凭空写；如果 Design 是主会话刚生成并已确认的文件，进入 Planning 时必须带上该方案文件路径，并在写计划前读取该文件；如果方案是用户直接提供的正文，计划必须以该正文为依据。
...
4.5. **泛化确认语义不能跨阶段复用：** `开始吧`、`继续`、`可以`、`按这个点进入改动流程` 这类表达，只能表示“同意进入实施入口”，不能等价成“已经选择 ACP 实施”。除非用户明确表达“交给 ACP / 委派实施 / 走 ACP”，否则主会话不得把这类泛化确认映射成 `implementation_executor=acp`。
...
4. 是否拦住，不看是不是一句话，也不看字数多少；只看当前信息是否足够支撑下一阶段。
5. 只有同时满足以下条件，才允许从入口继续流向 `design`：
   - 已稳定判定为“现有功能升级”或“新功能开发”。
   - 若为“新功能开发”，至少已经明确：业务目标、范围边界、成功标准、约束、优先级。
   - 若为“现有功能升级”，至少已经明确：受影响入口、现有流程范围、相关模块或代码线索、成功标准、约束或优先级。
   - 当前不存在关键前提缺失、上下文互相矛盾，或主会话无法稳定解释“为什么现在就可以进入方案”。
6. 只要缺少上一条任一关键项，就必须先停在任务定位、上下文补充或需求深挖，不得直接进入 `design`。
7. 面向用户解释为什么被拦住时，禁止把“因为你这是一句话/多句话”当成主理由；主理由必须明确落在“缺少哪些关键信息”。即使输入确实只有一句话，“一句话”也只能作为辅助事实，不能代替信息完整度判断。
8. 如果满足以下任一条件，必须先询问用户是否进入 `ian-think` 需求挖掘，不得直接进入 `design`：
   - 输入仅为“方法论文档/参考资料 + 一句话目标”。
   - 输入仅为“纯一句话新增需求”，例如“我要增加一个翻译功能，比如翻译截图里的文字，翻译当前选中行的文字。”。
   - 缺少最小业务信息（明确业务目标、边界、成功标准、约束、优先级）中的关键项。
   - 主会话无法稳定判定阶段或开发类型。
9. 对“现有功能升级”场景，若当前说不清涉及哪个现有流程、哪个入口、哪个模块，就先补充范围或代码线索，不得直接开始方案。
10. 对“新功能开发”场景，若当前只有一句话目标，或虽然不是一句话但仍缺少边界/验收/约束/优先级，也必须先补充最小业务信息，或进入需求深挖，不得直接开始方案。
11. 用户选择进入 `ian-think` 时，必须先完成“目标对齐”；目标对齐后还必须进入深度需求挖掘（优先 `brainstorming`，不可用则主会话按同结构兜底）。
12. 深度需求挖掘完成后，必须回填 `requirements_package`（`objective`、`user_ideas`、`business_scenarios`、`in_scope`、`out_of_scope`、`constraints`、`acceptance_criteria`、`risks`），缺任一项都不得进入 `design/planning`。
13. 用户不进入 `ian-think` 时，要求其直接补充上下文（文档内容或文档路径），再重新 `action=start`。
14. 不进入本地开发。
...
9. 主会话完成方案文档后，必须调用 `design_complete` 回填插件状态机，进入“方案确认”环节；未调用前不得口头宣布进入方案确认，更不得直接进入计划阶段。进入方案确认后，面向用户的确认提示必须包含：`如无补充请回复“可以/同意/确认”`。
10. 主会话完成计划文档后，必须调用 `planning_complete` 回填插件状态机，进入“计划确认”环节；未调用前不得口头宣布进入计划确认，更不得直接进入计划实施阶段。进入计划确认后，面向用户的确认提示必须包含：`如无补充请回复“可以/同意/确认”`。
...
15. 主会话完成方案并获得确认后，插件会返回计划执行方选择；若后续选择主会话继续写计划，必须按插件返回的 `planning_source` 读取方案文件并对齐同一路径文档来源。若返回结果要求把该方案文件路径写入 `requirement_text`，必须照做，不能省略。
...
4. 用户需要做的唯一选择（若有）。
5. 首屏文案必须按“四句结构”输出：当前阶段、进入原因、用户选择、选择影响。
6. 若当前处于“需求补齐/需求深挖前置”，进入原因必须直接点名缺失的关键信息，例如业务目标、范围边界、触发入口、成功标准、约束、优先级；禁止只写“这是一句话目标”或“信息太少”这类模糊理由。

入口异常（例如入口暂时不可继续、上下文不足、目录不可用）时，必须优先使用下面的人话模板：

1. 当前阶段：<业务阶段名称>。
2. 进入原因：<业务上为什么停在这里>。
3. 你现在要做的选择：<最多两个可执行选择>。
4. 选择影响：<每个选择会怎样影响后续流程>。

禁止把内部实现词当首屏主提示，包括但不限于：

1. 参数名（例如 `workspace_path`）。
2. 工具名（例如某个 MCP 工具名）。
3. 状态枚举（例如 `workflow_status`、`next_action_required`）。
